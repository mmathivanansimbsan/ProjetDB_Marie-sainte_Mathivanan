-- Liste les artistes Français et Américain
...
SELECT DISTINCT Nom_Scene
FROM Artiste
WHERE Nom_Scene LIKE '%DJ%'
ORDER BY Nom_Scene;

-- Liste les projets Pop ou Rock
SELECT Titre_Projet, Genre_Musical
FROM Projet
...
-- Les artistes ayant un nombre de stream supérieur à la moyenne
SELECT a.Nom_Scene, SUM(eps.Nombre_Streams) AS Total_Streams
FROM Artiste a
JOIN travaillent_sur ts ON a.ID_Artiste = ts.ID_Artiste
JOIN est_publie_sur eps ON ts.ID_Projet = eps.ID_Projet
GROUP BY a.Nom_Scene
...
SELECT p.Titre_Projet, p.Budget_Production
FROM Projet p
...
-- Classement de tous les artistes en fonction du nombre de récompenses différentes
SELECT a.Nom_Scene, COUNT(DISTINCT r.ID_Recompense) AS Nb_Recompenses
FROM Artiste a
...
-- classe les plateformes dont le nombre total de streams est supérieur à la musique la plus streamée
...
-- Genres musicaux ayant plusieurs projets et faisant partie des catégories récompensés.
